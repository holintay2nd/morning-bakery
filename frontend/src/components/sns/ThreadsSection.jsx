...
const ThreadsWordmark = () => (
  <img src="/threads_wordmark_black.png" alt="Threads" className="h-[22px] w-auto object-contain" draggable={false} />
)
...
export default function ThreadsSection({ items, username, profilePicture, tagline, followersCount, mediaCount }) {
  ...
  // 단일 프로필 행: @계정 + 게시물·팔로워 수 + 이동 버튼
  const profileEl = profileUrl ? (
    <div className="flex items-center gap-2 w-full">
      {showAvatar ? (
        <img src={profilePicture} alt={displayUsername} className="w-10 h-10 rounded-full object-cover flex-shrink-0" onError={() => setAvatarError(true)} />
      ) : (
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-bold text-gray-900 leading-tight truncate">@{displayUsername}</p>
        {(mediaCount != null || followersCount != null) && (
          <div className="flex gap-3 text-xs text-gray-400 mt-0.5">
            {mediaCount != null && (
              <span>게시물{' '}<strong className="text-gray-700">{mediaCount.toLocaleString()}</strong></span>
            )}
            {followersCount != null && (
              <span>팔로워{' '}<strong className="text-gray-700">{followersCount.toLocaleString()}</strong></span>
            )}
          </div>
        )}
      </div>
